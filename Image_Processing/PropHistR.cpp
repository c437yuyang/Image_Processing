// PropHistR.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CPropHistR, CPropertyPage)

CPropHistR::CPropHistR()
	: CPropertyPage(CPropHistR::IDD)
{
	CImage_ProcessingView *m_pView= (CImage_ProcessingView*)((CMainFrame*)AfxGetMainWnd())->GetActiveView();
	m_dMapHistR = m_pView->m_dMapHistR;
}
...
void CPropHistR::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CPropHistR, CPropertyPage)
	ON_WM_PAINT()
END_MESSAGE_MAP()


// CPropHistR message handlers


void CPropHistR::OnPaint()
{
	...
	//std::cout << dMapHistR.size() << std::endl;
